...
describe HansardReference, 'when finding a reference' do
  ...
  it 'should search for sections the first time it is asked to find sections' do 
    reference = default_reference
    reference.should_receive(:search_sections).and_return([])
    reference.find_sections
  end
  
  it 'should return a cached instance variable on the second request to find sections' do
    reference = default_reference
    section = mock_model(Section)
    reference.should_receive(:search_sections).and_return([section])
    reference.find_sections
    reference.should_not_receive(:search_sections)
    reference.find_sections.should == [section]
  end
  
  it 'should return the hansard reference suffix of its first found section when asked for a column suffix' do 
    reference = default_reference
    section = mock_model(Section, :sitting => HouseOfCommonsSitting.new)
    reference.stub!(:find_sections).and_return([section])
    HouseOfCommonsSitting.stub!(:hansard_reference_suffix).and_return('suffix')
    reference.column_suffix.should == 'suffix'  
  end
  
  it 'should return nil when asked for a column suffix if it doesn\'t find any sections' do
    reference = default_reference
    reference.stub!(:find_sections).and_return([])
    reference.column_suffix.should be_nil
  end
  
  it 'should find section for date and column reference that matches a Commons section' do
    column = 580
    date = Date.new(1986, 1, 15)
    reference = make_reference column, date, nil
    section = mock(Section)
    HouseOfCommonsSitting.stub!(:find_section_by_column_and_date).with(column, date, @end_column).and_return(section)
    HouseOfLordsSitting.stub!(:find_section_by_column_and_date).with(column, date, @end_column).and_return(nil)
    reference.find_sections.should == [section]
    reference.house.should == nil
  end

  it 'should find section for date and column reference that matches a Lords section' do
    column = 580
    date = Date.new(1986, 1, 15)
    reference = make_reference column, date, nil
    section = mock(Section)
    HouseOfCommonsSitting.should_receive(:find_section_by_column_and_date).with(column, date, @end_column).and_return(nil)
    HouseOfLordsSitting.should_receive(:find_section_by_column_and_date).with(column, date, @end_column).and_return(section)
    reference.find_sections.should == [section]
    reference.house.should == nil
  end

  it 'should find sections for date and column reference that matches both a Lords and a Commons section' do
    column = 580
    date = Date.new(1986, 1, 15)
    reference = make_reference column, date, nil
    commons_section = mock(Section)
    lords_section = mock(Section)
    HouseOfCommonsSitting.should_receive(:find_section_by_column_and_date).with(column, date, @end_column).and_return(commons_section)
    HouseOfLordsSitting.should_receive(:find_section_by_column_and_date).with(column, date, @end_column).and_return(lords_section)
    sections = reference.find_sections
    sections.include?(commons_section).should be_true
    sections.include?(lords_section).should be_true
    reference.house.should == nil
  end

  it 'should find section for Commons reference' do
    column = 1090
    date = Date.new(2005,12,2)

    reference = make_reference column, date, :commons
    section = mock(Section)
    HouseOfCommonsSitting.should_receive(:find_section_by_column_and_date).with(column, date, @end_column).and_return(section)

    reference.find_sections.should == [section]
  end

  it 'should find section for Lords reference' do
    column = 1090
    date = Date.new(2005,12,2)

    reference = make_reference column, date, :lords
    section = mock(Section)
    HouseOfLordsSitting.should_receive(:find_section_by_column_and_date).with(column, date, @end_column).and_return(section)
  ...
  it 'should find section for Commons written answer reference' do
    column = 1090
    date = Date.new(2005,12,2)
    reference = make_reference column, date, :commons, :written_answer=>true
    section = mock(Section)
    CommonsWrittenAnswersSitting.should_receive(:find_section_by_column_and_date).with(column, date, @end_column).and_return(section)
    reference.find_sections.should == [section]
  end

  it 'should find section for Commons Westminster Hall reference' do
    column = 1090
    date = Date.new(2005,12,2)
    reference = make_reference column, date, :commons, :westminster_hall=>true
    section = mock(Section)
    WestminsterHallSitting.should_receive(:find_section_by_column_and_date).with(column, date, @end_column).and_return(section)
    reference.find_sections.should == [section]
  end

  it 'should find section for Lords written answer reference' do
    column = 1090
    date = Date.new(2005,12,2)
    reference = make_reference column, date, :lords, :written_answer=>true
    section = mock(Section)
    LordsWrittenAnswersSitting.should_receive(:find_section_by_column_and_date).with(column, date, @end_column).and_return(section)
    reference.find_sections.should == [section]
  end

  it 'should find sections for date and column reference that matches both a Lords and a Commons written answers section' do
    column = 580
    date = Date.new(1986, 1, 15)
    reference = make_reference column, date, nil, :written_answer=>true
    commons_section = mock(Section)
    lords_section = mock(Section)
    CommonsWrittenAnswersSitting.should_receive(:find_section_by_column_and_date).with(column, date, @end_column).and_return(commons_section)
    LordsWrittenAnswersSitting.should_receive(:find_section_by_column_and_date).with(column, date, @end_column).and_return(lords_section)
    sections = reference.find_sections
    sections.include?(commons_section).should be_true
    sections.include?(lords_section).should be_true
    reference.house.should == nil
  ...
  it 'should find a section for date and column reference that matches a written answers section with no house' do
    column = 580
    date = Date.new(1986, 1, 15)
    reference = make_reference column, date, nil, :written_answer=>true
    section = mock(Section)
    CommonsWrittenAnswersSitting.should_receive(:find_section_by_column_and_date)
    LordsWrittenAnswersSitting.should_receive(:find_section_by_column_and_date)
    WrittenAnswersSitting.should_receive(:find_section_by_column_and_date).with(column, date, @end_column).and_return(section)
    reference.find_sections.should == [section]
    reference.house.should == nil
  end

  it 'should  find section for Lords written statement reference (we have parsed these yet?)' do
    column = 1090
    date = Date.new(2005,12,2)
    reference = make_reference column, date, :lords, :written_statement=>true
    section = mock(Section)
    LordsWrittenStatementsSitting.should_receive(:find_section_by_column_and_date).with(column, date, @end_column).and_return(section)
    reference.find_sections.should == [section]
  end
